...
engine_sp500_at = create_engine('mysql+pymysql://root:123456@localhost:3306/AT_Model')
...
def savedt():
    ...
    df_js225 = pd.read_sql_query(sql_sp500_at, engine_sp500_at)
    excelFile3 = '{0}/{1}.xlsx'.format(ln, "sp500_AT")  # 处理了文件属于当前目录下！
...
def sendmail():
    ...
    title = 'AT_SP500_'  # 邮件主题
    content = 'AT_SP500_'  # 邮件内容
    ...
    try:
        ...
        xlsxpart3 = MIMEApplication(open('{0}/sp500_AT.xlsx'.format(ln), 'rb').read())
        xlsxpart3.add_header('Content-Disposition', 'attachment', filename='sp500_AT.xlsx')
